...
// visited.
  ...
  return next.first >= 0 && next.first < (int) image.width() && next.second >= 0 && next.second < (int)image.height() && D[next.second][next.first] == -1 && closeEnough(*image.getPixel(curr.first, curr.second), *image.getPixel(next.first, next.second));
...
// level by level. The distance of a pixel from "start" is its height
...
// 33 = (9+1+4+1+3+1)+9+1+4.
  ...
  //RGBAPixel *startPixel = image.getPixel(start.first, start.second);
  ...
  unordered_map<int,RGBAPixel> vals;

  // distance before pattern repeats
  // in the example it is (9 + 1 + 4 + 1 + 3 + 1) = 19
  int total = 0;

  for (auto &p : vgc) {
    vals[p.first + total] = p.second;
    total += p.first + 1;
  }

  pair<int, int> curr = start;
  q.enq(curr);
  D[curr.second][curr.first] = 0;

  while (!q.empty()) {

    curr = q.deq();

    for (auto &neighbour : neighbors(curr)) {
      if (good(image, D, curr, neighbour)) {
        q.enq(neighbour);
        D[neighbour.second][neighbour.first] = D[curr.second][curr.first] + 1;
      }
    }

    if (vals.count(D[curr.second][curr.first] % total)) {
      *image.getPixel(curr.first,curr.second) = vals[D[curr.second][curr.first] % total];
    }
  }
